...
func (t *lazyTimer) Chan() <-chan time.Time {
	if t.timer == nil {
		panic("cannot call Chan() on lazy timer before calling Reset")
	}
	return t.timer.Chan()
}

func (t *lazyTimer) Reset(d time.Duration) bool {
	if t.timer == nil {
		t.timer = t.clk.NewTimer(d)
		return false
	}
	return t.timer.Reset(d)
}

func (t *lazyTimer) Stop() bool {
	if t.timer == nil {
		return true
	}
	return t.timer.Stop()
