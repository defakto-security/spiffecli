...
func (s *TokenSuite) SetupTest() {
	s.bundles = jwtbundle.FromJWTAuthorities(
...
func (s *TokenSuite) TestSignAndValidate() {
	...
	for _, testCase := range testCases {
		...
		s.T().Run(testCase.kid, func(t *testing.T) {
			...
			spiffeID, claims, err := ValidateToken(ctx, token, s.bundles, fakeAudience[0:1])
			require.NoError(t, err)
...
func (s *TokenSuite) TestSignAndValidateWithAudienceList() {
	...
	spiffeID, claims, err := ValidateToken(ctx, token, s.bundles, fakeAudiences[0:1])
	s.Require().NoError(err)
...
func (s *TokenSuite) TestValidateNoSubject() {
	...
	spiffeID, claims, err := ValidateToken(ctx, token, s.bundles, []string{"FOO"})
...
func (s *TokenSuite) TestValidateUnexpectedAudienceList() {
	...
	spiffeID, claims, err := ValidateToken(ctx, token, s.bundles, []string{"AUDIENCE3"})
